...
<html>
<head>
    <title>Payment Credit Card The Executive</title>
...
<body>
    ...
    <table id="example" class="display" cellspacing="0" width="100%">
        ...
        <tbody>
        <?php
        $sql = "SELECT distinct sog.entity_id, sog.increment_id, sog.grand_total, sog.created_at, sog.billing_name, sog.shipping_information, LEFT(sog.shipping_information,LOCATE(' ',sog.shipping_information) - 1) as kurir, sog.shipping_and_handling, sog.shipping_name,
            ...
            WHERE (a.parent_item_id is null) and soa.address_type='shipping' and sog.payment_method='ipay88_cc' and sog.status = 'processing' and
            sog.created_at BETWEEN NOW() - INTERVAL 8 DAY AND NOW()";

// SELECT distinct sog.entity_id, sog.increment_id, sog.grand_total, sog.created_at, sog.billing_name, sog.shipping_information, LEFT(sog.shipping_information,LOCATE(' ',sog.shipping_information) - 1) as kurir, sog.shipping_and_handling, sog.shipping_name,
// soa.street, soa.city, SUBSTRING_INDEX(soa.city, ',', -1) as city1, soa.postcode, soa.telephone, soa.region,
// a.order_id, a.sku, a.qty_ordered, a.name as productname, b.price, b.base_price, b.base_original_price, b.base_original_price-b.price AS discount, b.discount_amount AS 'DiscountAmount', b.price - b.discount_amount as net_amount, so.base_discount_amount as diskonkupon
// FROM sales_order_item AS a 
// LEFT JOIN sales_order_item AS b on ((a.order_id = b.order_id) and (a.sku=b.sku) and (b.parent_item_id is null))
// LEFT JOIN sales_order_grid sog ON sog.entity_id=a.order_id
// LEFT JOIN sales_order_address soa ON sog.entity_id=soa.parent_id
// LEFT JOIN sales_order so ON a.order_id=so.entity_id
// WHERE (a.parent_item_id is not null) and sog.payment_method='ipay88_cc' and sog.status !='canceled' and sog.created_at BETWEEN NOW() - INTERVAL 8 DAY AND NOW()
        $query = $db->query($sql);
        $noUrut=1;
