...
<html>
<head>
    <title>Payment Confirmation Receive The Executive</title>
...
<body>
    ...
    <table id="example" class="display" cellspacing="0" width="100%">
        ...
        a.order_id, a.sku, a.qty_ordered, a.name as productname, a.price, a.base_price, a.base_original_price, a.base_original_price-a.price AS discount, 
        a.discount_amount AS 'DiscountAmount', a.price - a.discount_amount as net_amount, so.base_discount_amount as diskonkupon
        ...
        WHERE (a.parent_item_id is null) and sog.payment_method='banktransfer' and sog.status ='paymentconfirmationreceive' and soa.address_type = 'shipping' and
        sog.created_at BETWEEN NOW() - INTERVAL 20 DAY AND NOW()";
...
// b.discount_amount AS 'DiscountAmount', b.price - b.discount_amount as net_amount
// FROM sales_order_item AS a 
// LEFT JOIN sales_order_item AS b on ((a.order_id = b.order_id) and (a.sku=b.sku) and (b.parent_item_id is null))
// LEFT JOIN sales_order_grid sog ON sog.entity_id=a.order_id
// LEFT JOIN sales_order_address soa ON sog.entity_id=soa.parent_id
// WHERE (a.parent_item_id is not null) and sog.payment_method='banktransfer' and sog.status ='paymentconfirmationreceive' 
// and sog.created_at BETWEEN NOW() - INTERVAL 13 DAY AND NOW()
        ...
        ?>
            <tr>
                ...
                <td><?php echo $row['base_original_price'];?></td>
                <td><?php echo $row['discount'];?></td>
                <td><?php echo number_format($diskon,0,",","");?></td>
                <td><?php echo $row['net_amount'];?></td>
                <td><?php echo $row['shipping_and_handling'];?></td>
