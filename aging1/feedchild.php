...
if(mysqli_connect_errno()) {
    ...
    $sql = $db->query("SELECT a.entity_id, a.sku, b.product_id, b.is_in_stock, c.value as name, f.value as warna, g.value as deskripsi 
        ...
        WHERE b.min_qty >= 0.0000 AND b.product_id != 'NULL' AND c.attribute_id = 84 AND c.store_id = 0 AND e.attribute_id = 93 
        AND f.store_id = 0 AND g.attribute_id = 76 AND g.store_id = 0 AND b.is_in_stock != 0 ORDER BY a.entity_id DESC");

    $delimiter = ",";
    $filename = "Facebook-Catalog-Child-Executive.csv";
    $f = fopen('php://memory', 'w');
    $fields = array('ID','Product ID','Availability','Condition','Title','Title English','Title Translate','Description','Link','Image Link','Price','Gtin','MPN','Brand',
        'Item Group Id','Item Group Id Translate','Google Product Category','size','color','color Translate');
    ...
    while ($row = mysqli_fetch_array($sql)){
        ...
        if(substr($row4['rule_price'], 0, -2) == 0){
            $sql7 = $db->query("SELECT price FROM catalog_product_flat_1 WHERE entity_id = $id");
        ...
        $lineData = array(
            ...
            'new', 
            ucwords($name), 
            'The Executive '.ucwords(strtolower($row5['value'])), 
            'The Executive '.$googletranslate, 
            $desc, 
            $link,
            $image,
            $harga.' IDR', 
