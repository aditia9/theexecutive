...
class Data extends \Magento\Framework\App\Helper\AbstractHelper
{
    ...
    public function getEntryTitle($product)
    {
        ...
        switch ($this->titleAttr) {
            case 'name':
                $title2 = $product->getName();
                $title1 = explode (" - ",$title2);
                $title  = ucfirst(strtolower($title1[0]));   
                break;
            default:
                $title2 = $this->getAttributeValue($product, $this->titleAttr);
                $title1 = explode (" - ",$title2);
                $title  = ucfirst(strtolower($title1[0]));
        }
    ...
    public function getEntryDescription($product)
    {
        ...
        if (!$description) {
            ...
            foreach ($priority as $attr) {
                $description = ucfirst(strtolower($this->getDescriptionValue($product, $attr)));
    ...
    public function getEntryImageLink($product)
    {
        ...
        if ($this->isValidUrl($imageUrl)) {
            ...
            // Placeholder image
            $this->productImageUrl = $this->viewAssetRepo->getUrl(
